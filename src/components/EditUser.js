...
function AddNewItem({ user }) {
  ...
  const [mondayAccess, setMondayAccess] = useState(user.access.monday);
  const [tuesdayAccess, setTuesdayAccess] = useState(user.access.tuesday);
  const [wednesdayAccess, setWednesdayAccess] = useState(user.access.wednesday);
  const [thursdayAccess, setThursdayAccess] = useState(user.access.thursday);
  const [fridayAccess, setFridayAccess] = useState(user.access.friday);
  const [saturdayAccess, setSaturdayAccess] = useState(user.access.saturday);
  const [sundayAccess, setSundayAccess] = useState(user.access.sunday);
  ...
  return (
    <BaseTemplate title={user.name}>
      {user && <div className="edit-user">
        <IonGrid>
          <IonRow>
            <IonCol offset-md="1" size-md="10">
                  <h1 style={{margin: '1em auto'}}><strong>Edit User</strong></h1>
...
export default connect(mapStateToProps, { fetchUser })(AddNewItem);
